...
# Tables print LaTeX via knitr::kable when available; fall back to a no-op if
# knitr is not installed (figures do not depend on it).
...
ggsave(
  here::here("Figures", "Color", "Retrospective", "sim_vs_real_deaths.pdf"),
  width = 8,
  height = 5
)
...
ggsave(
  here::here("Figures", "Color", "Real_Time", "sim_vs_real_deaths.pdf"),
  width = 8,
  height = 5
)
...
pPop1 <- ggplot(
  ...
  aes(x = pop, y = mae, color = est_name)
) +
  # geom_smooth(se = FALSE, method = "gam", formula = y ~ s(x, bs = "cs")) +
  geom_point(stroke = NA, alpha = 0.7, size = 3) +
  scale_color_manual(
    breaks = c("Conv Ratio", "Lagged Ratio", "Deconv-0"),
    values = colours
  ) +
  labs(x = "Population", y = "MAE", color = "") +
  theme_bw() +
...
ggsave(
  here::here("Figures", "Color", "Retrospective", "improvements.pdf"),
  width = 8,
  height = 4
)
...
p1 <- ggplot(
  ...
  aes(time_value, est, color = est_name, alpha = est_name)
) +
...
ggsave(
  here::here("Figures", "Color", "Retrospective", "single_curve.pdf"),
  width = 8,
  height = 4
)
...
ggsave(
  here::here("Figures", "Color", "Real_Time", "improvements.pdf"),
  width = 8,
  height = 4
)
...
ggsave(
  here::here("Figures", "Color", "Real_Time", "single_curve.pdf"),
  width = 8,
  height = 4
)
...
all_right %>%
  ...
  ggplot(aes(offset, m, color = est_name)) +
  geom_line() +
  geom_point() +
...
ggsave(here::here("Figures", "Color", "misspecified_maes.pdf"), width = 8, height = 4)
...
ggsave(
  here::here("Figures", "Color", "misspecified_improvements.pdf"),
  width = 8,
  height = 4
)
...
ggsave(
  here::here("Figures", "Color", "real-data-pa.pdf"),
  width = 8,
  height = 4
)
...
ggsave(
  here::here("Figures", "Color", "misspecified_delay_distrs.pdf"),
  width = 8,
  height = 4
)
...
ggsave(
  here::here("Figures", "Color", "Retrospective", "all_curves.pdf"),
  width = 11,
  height = 15,
  dpi = 300
)
...
ggsave(
  here::here("Figures", "Color", "Real_Time", "all_curves.pdf"),
  width = 11,
  height = 15,
  dpi=300
)
...
ggsave(here::here("Figures", "Color", "tail-regularization-comparison.pdf"),
       width = 10, height = 5)
...
ggsave(here::here("Figures", "Color", "runtime-comparison.pdf"),
       width = 8, height = 4)


# Time-varying delay experiment (Appendix) --------------------------------
# These figures read committed results. The underlying MAE summary is produced
# by Code/Analysis/maes-time-var.R (run as part of the data pipeline); the
# curves/delays read time-var-delay.rds and the raw data directly. Each block
# runs in local() so its temporaries do not leak into the global environment.
tvar_out <- here::here("Figures", "Color", "time-varying")

# Figure - Time-varying MAE summary
local({
  ...
  p <- ggplot(tvar_df, aes(x = geo_value, y = mean_mae, fill = method)) +
    ...
    scale_fill_manual(
      name = NULL,
      values = c(
        "Deconv-0" = "#009E73", "Deconv-1" = "#CC79A7", "Deconv-2" = "#0072B2",
        "Conv. ratio" = "#E69F00", "Lagged ratio" = "#56B4E9"
      )
    ) +
...
# Figure - Time-varying curves (retrospective, well-specified delay)
local({
  ...
  state_labels <- c("CA", "LA", "PA", "UT")
  method_colors <- c(
    "Deconv-0" = "#009E73", "Deconv-1" = "#CC79A7",
    "Deconv-2" = "#0072B2", "Conv. ratio" = "#E69F00"
  )
  ...
  p <- ggplot(d, aes(time_value, est, color = method)) +
...
# Figure - Time-varying delay illustration
local({
  delay_means <- tribble(
    ~variant,   ~retro, ~rt,
    "Original",  8,     18,
  ...
  variant_colors <- c(
    "Original" = "#A6CEE3", "Alpha" = "#1F78B4",
    "Delta" = "#B2DF8A", "Omicron" = "#33A02C"
  )
  date_start <- as.Date("2021-01-01")
  date_end   <- as.Date("2022-04-01")
  dd <- 60L
  ...
  p_top <- ggplot(pmfs |> filter(day <= plot_max_day),
                  aes(day, prob, color = variant)) +
    geom_line(linewidth = 0.7) +
    facet_wrap(~ problem, nrow = 1) +
    scale_color_manual(values = variant_colors, name = NULL) +
  ...
  p_bot <- ggplot(props, aes(time_value, prop, fill = variant)) +
    ...
    scale_fill_manual(values = variant_colors, name = NULL, guide = "none") +
